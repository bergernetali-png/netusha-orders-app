...
function calcOrder(order, settings) {
  ...
  // אחוז הרווח "תכל'ס" — לפני זמן עבודה. זה הרווח הראשי שמוצג בדשבורד וברשימת ההזמנות,
  // כי זה מה שבאמת נכנס לכיס; עלות זמן העבודה נשארת ככלי לתמחור בפירוט בתוך ההזמנה עצמה.
  const productMarginPct = sellTotal > 0 ? (productProfit / sellTotal) * 100 : 0;

  return {
    sellTotal, materialsCost, brandingCost, prepCost, shippingCost, otherCosts,
    productCost, productProfit, totalCost, profit, marginPct, productMarginPct
  };
}
